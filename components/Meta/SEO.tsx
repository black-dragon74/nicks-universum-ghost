...
import { siteDescriptionMeta, siteIcon, siteTitleMeta } from "./defaults"
...
export const SEO = (props: SEOProps) => {
  const { title: t, description: d, settings, seoImage, article } = props
  const {
    og_title,
    og_description,
    published_at,
  ...
  const canonical = resolve(siteUrl, asPath)
...
const getJsonLd = ({
  ...
  const siteURL = settings.processEnv.siteURL
  const pubLogoURL = settings.logo || resolve(siteURL, siteIcon)
  const type = article ? "Article" : "Website"
